...
def clip_grad_by_norm_(grad, max_norm):
        ...
        for g in grad:
            ...
            total_norm = total_norm.clone() + param_norm.item() ** 2
            counter += 1
...
def train_task(args, train_loader, current_task, prototype={}, pre_index=0):
    ...
    if 'miniimagenet' in args.data:
        model = models.create('resnet18_imagenet', pretrained=False, feat_dim=args.feat_dim,embed_dim=args.num_class,norm=True)
...
# db = DataLoader(mini, args.BatchSize, shuffle=True, num_workers=1, pin_memory=True)
        ...
        for step, (x_spt, y_spt, x_qry, y_qry) in enumerate(train_loader):
            ...
            losses_q = [0 for _ in range(args.update_step)]  # losses_q[i] is the loss on step i
            corrects = [0 for _ in range(args.update_step)]
            
            y_onehot_f = torch.cuda.FloatTensor(setsz, args.num_class)
            # y_onehot_pre = torch.cuda.FloatTensor(setsz, args.num_class)
            
            for i in range(args.BatchSize):
                #x_spt[i], y_spt[i], x_qry[i], y_qry[i] = x_spt[i].to(device), y_spt[i].to(device), x_qry[i].to(device), y_qry[i].to(device)
                ...
                for k in range(0, args.update_step):
                    ...
                    if current_task > 0: 
                        ...
                        if args.mean_replay:
                            ...
                            y_onehot_f.zero_()
                            y_onehot_f.scatter(1, embed_label_sythesis[:, None], 1)
                            syn_label_pre = y_onehot_f.to(device)
            ...
            # end of all meta tasks
            # sum over all losses on query set across all meta tasks
            ...
            scheduler.step()
            # optimizer_mlp.step()
            accs = np.array(corrects) / (querysz * BatchSize)
            # print(accs[-1])
        ...
        print('[Metric Epoch %05d]\t Total Loss: %.3f \t LwF Loss: %.3f \t Query Loss: %.3f \t Query Accuracy FeatureX: %.3f \t'
                % (epoch + 1, loss_log['C/loss'], loss_log['C/loss_aug'], loss_log['C/loss_cls_q'], accs[-1]))
                ...
                for i in range(args.BatchSize): # This is inner loop not task
                    ...
                    for k in range(args.update_step):
                        ...
                        for e,param in enumerate(discriminator.parameters(),0):
                            param.data = param.data.clone() - args.update_lr * grad_d[e]
                        for e,param in enumerate(generator.parameters(),0):
                            param.data = param.data.clone() - args.update_lr * grad_g[e]
                        ...
                        d_losses_q[k] += d_loss_lbls_q  # d_losses_q[k] + 
                        ...
                        g_losses_q[k] += g_loss_lbls_q # g_losses_q[k] + 
                        
                #with torch.autograd.detect_anomaly():
                #optimizer_D.zero_grad()
                #optimizer_G.zero_grad()
                d_loss_q_total = d_losses_q[-1]/args.BatchSize
                g_loss_q_total = g_losses_q[-1]/args.BatchSize
    ...
    for i in range(args.start, num_task+1):
        ...
        if args.data == 'cifar100':
            ...
            trainfolder = MiniImagenet(traindir, mode='train', n_way=args.n_way, k_shot=args.k_spt,
                        k_query=args.k_qry,
                        batchsz=5000, resize=args.img_sz, cls_index=class_index)
